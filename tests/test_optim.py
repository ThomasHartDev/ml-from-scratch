...
def test_minimize_quadratic_finds_minimum():
    # f(x) = (x - 3)^2 has its minimum at x = 3
    x = Value(-4.0)
...
def test_momentum_reaches_minimum():
    ...
    # momentum should reach the basin faster than plain SGD at the same lr
    y = Value(-4.0)
...
def test_linear_regression_recovers_weights():
    # fit y = w*x + b to points generated from w=2, b=-1 via pure autograd
